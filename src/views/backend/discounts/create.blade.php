...
<div class="container min-height pb-3">
        ...
        <div class="col-sm-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mt-3">
                    <li class="breadcrumb-item"><a href="{{ route('dashboard.module.order_form.discounts.index') }}">Kortingen & Coupons</a></li>
    ...
    <form action="{{ route('dashboard.module.order_form.discounts.save') }}" method="POST">
        <div class="row tab-content bg-light shadow-sm rounded p-3 mb-3 mx-1" id="pageTabContent">
            ...
            <div class="col-sm-12 tab-pane fade show active" id="d_general" role="tabpanel" aria-labelledby="d_general-tab">
              @include('chuckcms-module-order-form::backend.discounts.create._tab_general')
            </div>


            <div class="col-sm-12 tab-pane fade" id="d_conditions" role="tabpanel" aria-labelledby="d_conditions-tab">
              @include('chuckcms-module-order-form::backend.discounts.create._tab_conditions')
            </div>


            <div class="col-sm-12 tab-pane fade" id="d_actions" role="tabpanel" aria-labelledby="d_actions-tab">
              @include('chuckcms-module-order-form::backend.discounts.create._tab_actions')  
            </div>
...
$(document).ready(function() {
    ...
    $('body').on('click', '#addCouponsToUncompatibleBtn', function (event) {
        event.preventDefault();

        $('select[name="compatible_discounts"]').find('option:selected').each(function () {
            $(this).clone().appendTo('select[name="uncompatible_discounts[]"]');
            $(this).attr('disabled', true);
            $(this).addClass('d-none');

            if ($('input[name="uncompatible_discounts_concatenated"]').val().length > 0) {
                $('input[name="uncompatible_discounts_concatenated"]').val($('input[name="uncompatible_discounts_concatenated"]').val()+','+$(this).val());
            } else {
                $('input[name="uncompatible_discounts_concatenated"]').val($(this).val());
            }
        });
    });

    $('body').on('click', '#addCouponsToCompatibleBtn', function (event) {
        event.preventDefault();

        $('select[name="uncompatible_discounts[]"]').find('option:selected').each(function () {
            discount_id = $(this).val();
            $('select[name="compatible_discounts"]').find('option').each(function() {
                if ($(this).val() == discount_id) {
                    $(this).prop('disabled', false);
                    $(this).removeClass('d-none');
                }
            });

            all_discounts = $('input[name="uncompatible_discounts_concatenated"]').val().split(',');
            new_all_discounts = [];
            for (var i = 0; i < all_discounts.length; i++) {
                if (all_discounts[i] !== discount_id) {
                    new_all_discounts.push(all_discounts[i]);
                }
            };

            $('input[name="uncompatible_discounts_concatenated"]').val(new_all_discounts.join(','));
    ...
    function refreshDiscountCode() {
        return $.ajax({
            method: 'POST',
            url: "{{ route('dashboard.module.order_form.discounts.refresh_code') }}",
            data: { 
                _token: token
            }
        });
