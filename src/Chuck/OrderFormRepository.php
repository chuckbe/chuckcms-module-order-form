...
class OrderFormRepository
{
    ...
    {
        if(config('chuckcms-module-order-form.order.payment_upfront')) {
            $total = $this->formEntry->where('slug', config('chuckcms-module-order-form.products.slug'))->where('entry->status', 'paid')->whereDate('entry->order_date', '>', Carbon::today()->subDays(7))->sum('entry->order_price');
        } else {
    ...
    {
        if(config('chuckcms-module-order-form.order.payment_upfront')) {
            return $this->formEntry->where('slug', config('chuckcms-module-order-form.products.slug'))->where('entry->status', 'paid')->where('entry->order_date', '>', Carbon::today()->subDays(7))->count();
        } else {
